...
VH3 AI for field service aims to be what Cursor is to software development. Not a replacement. A harness.
...
Every customer's data is scoped entirely to their organisation. All API calls resolve server-side by `company_id`, no call can cross organisation boundaries. The intelligence you build is yours: query it via the API, consume it through automations, connect it to your own AI models, or build your own applications directly on top of it. Dedicated, isolated infrastructure is available on Enterprise plans.
...
<Steps>
  ...
  <Step title="Connect and index">
    Enriched records flow into all four memory primitives simultaneously. The intelligence layer links every entity relationally. The semantic index makes meaning-based fault discovery possible across millions of job notes. Structured records are compacted and ready for any downstream consumer. Temporal sentinels begin watching from the first record. Every query, relational, semantic, structured, or pattern-based, works against the same underlying data.
  </Step>
  <Step title="Monitor">
