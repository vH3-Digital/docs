...
<AccordionGroup>
  <Accordion title="Who is Connie?">
    ...
    She is not a chatbot over exports. She handles multi-turn operational reviews, account prep, diagnostics, and follow-through that depends on context from earlier in the session.
  </Accordion>
  ...
  <Accordion title='What does "bring your own key" (BYOK) mean?'>
    ...
    This is different from most AI products, where the vendor buys tokens, marks them up, and bundles the cost somewhere invisible. With BYOK, there is no layer between you and the model. You can switch models, adjust usage, and see the full economics. If you build your own agents or automations via the API or n8n, you can use any model, harness, or framework you choose.
  </Accordion>
  ...
  <Accordion title='What are "tools" in agent terms?'>
    Structured API operations — search, aggregate, investigate, job lookup, reports, briefings, and more — that the AI invokes with validated parameters rather than freeform text. The tool surface is an ever-growing set that gives Connie and your custom agents precise, reliable access to the intelligence layer.
  </Accordion>
