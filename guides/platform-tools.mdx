...
Cases + teams turn detection into owned work. This is what makes "autonomous in the background" real instead of marketing.
...
**Pre-compute, do not re-derive.** Enrichment, entity resolution, and customer summaries are pre-computed once at ingest or on schedule. Read them from the model; do not re-extract on every call.

**Use cases for follow-through, not memos.** Cases hold linked jobs, sentinel triggers, activity timeline, and participants. They are the audit trail and the assignment surface. They are not a place to keep prose notes that should live in a job worksheet.
...
3. **No-LLM detection.** Sentinels and dashboard read the graph directly; you do not need to budget tokens to find out what changed.
