...
| Jobs feed | `GET /jobs/feed` | Paginated job list with structured filters (contact, date, type, status, engineer, punctuality) and sort by timing deltas | Account 360 views, workload audits, worst late-job lists, account-hierarchy roll-ups |
| Job aggregate | `POST /aggregate/jobs` | Aggregations: job count, completion rate, first-visit fix rate, avg start/end deltas, punctuality filters, grouped by engineer / type / period | KPIs, late-job counts, comparisons, charts |
...
| Account summary by parent contact | `GET /search/account-summary/by-parent/{company_id}/{contact_id}` | Roll every child Customer Summary under a parent contact into one account-level brief | Portfolio QBRs, "how is the whole account doing" |
| Account summary section search | `POST /search/account-summary-sections` | Search account-level knowledge sections by meaning | Cross-account portfolio questions |
...
| Report generation | `POST /reports/generate` | Eight cadence types: `start_of_day`, `midday`, `close_of_business`, `day_review`, `start_of_week`, `midweek`, `end_of_week`, `account_monthly` | Operational rhythms, account reviews, QBRs |
...
| Tool | Endpoint | What it does |
|------|----------|--------------|
| Cases | `/cases/*` | Create cases, attach jobs and sentinel triggers, transition status, log activity, assign participants, set Assigned Team (`POST /cases/{case_id}/assign-team`). Connie: `cases_for_entity`, `list_cases` (company catalogue; drafts excluded from list), `get_case`, `create_case`, `annotate_case`, `assign_case_team` |
| Teams | `/teams/*` | Create teams, link entities (customers, sites, regions), add members, scope visibility. Connie: `teams_for_entity`, `teams_for_user`, `list_teams` (company catalogue), `get_team` (members + entity links) |
