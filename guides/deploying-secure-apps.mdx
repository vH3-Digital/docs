...
Governance on this platform is not a separate product. It is the user, team, and case model used correctly.
...
1. A **sentinel** detects a pattern continuously (no LLM cost at detection).
...
These are **access points for different readiness**, not a ladder. A team that already has a strong front-end may start on Core and build its own UI. A team that wants to skip UI work uses Connect. The substrate is the same.
