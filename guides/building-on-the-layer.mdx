...
For parent contacts with multiple child sites (a retail chain, a housing association), the **Account Summary** rolls every child Customer Summary up into one account-level brief, refreshed the same way. Fetch it with `GET /search/account-summary/by-parent/{company_id}/{contact_id}`, or search across account-level sections with `POST /search/account-summary-sections`. Pass any child contact ID and the lookup resolves to the parent automatically — a customer 360 app does not need to look up the parent contact first. See [Operational discovery: Account Summary](/guides/operational-discovery#account-summary-the-whole-account-not-just-one-site) for the full shape.
