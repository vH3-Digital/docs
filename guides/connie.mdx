...
<Card title="Do this in Connect" icon="laptop" href="/connect/ask-connie">
  Open Ask Connie in VH3 Connect, manage sessions, and work in the app UI.
</Card>
...
- **Account Summary** rolls every child Customer Summary under a parent contact into one account-level brief, kept current the same way. Ask about a child site and Connie answers for that site; ask about the account and she resolves up to the parent automatically.
...
- "How is the whole Tesco account doing, not just Watford?" (Account Summary rollup)
...
- **Cases** turn signals into owned work with status, participants, and linked jobs. Connie can browse the company Case catalogue, open a case, attach evidence, annotate with comments, and set or clear the Case’s Assigned Team through the Intelligence API.
- **Teams** route cases to the right people: regional, portfolio, or functional groups own customers and sites so nothing sits in a generic queue. Connie can list Teams, load members and entity links, and find which Team owns a customer or site.
