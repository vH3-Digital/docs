...
This is not just hygiene. Shorter, focused sessions perform better in three measurable ways:
...
Sessions are durable. Past sessions can be retrieved, listed, and searched via the [Connie API](/api-reference/connie). You do not need to keep a single long thread alive to preserve evidence; the platform preserves it for you.
...
This is not "the model retraining itself in the background." It is **your organisation's intelligence layer getting richer continuously** and Connie reading that layer on every turn, including what happened earlier in the session.
...
- **Tenant-level prompt caching.** Stable instruction blocks (system prompts, tool schemas, company operating context) are cached across all users in your organisation. Every person on the team benefits from cache hits that earlier sessions already warmed. `cacheReadTokens` in the usage response shows how much of each turn was served from cache rather than re-computed.
...
Most of Connie's value lands without anyone typing. Connie reads the same signals the platform watches continuously, so she can pick up work the operator has not asked about yet.

- **Sentinels** detect patterns 24/7 (no LLM cost at detection): repeat visits, SLA drift, dormant customers, performance slips. See [Sentinels](/guides/sentinels).
...
Connie orchestrates **specialist delegates** for focused work (for example structured email generation and deeper analytical passes). You interact with one assistant; the platform routes complex sub-tasks without you managing multiple agents.
