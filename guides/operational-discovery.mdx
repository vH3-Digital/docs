...
<Card title="Do this in Connect" icon="laptop" href="/connect/search">
  Use header search and advanced search in VH3 Connect.
</Card>
...
## Account Summary (the whole account, not just one site)

A parent contact rolls up more than one child site: a retail chain with dozens of stores, a housing association with dozens of blocks. The **Account Summary** pulls every child Customer Summary, the account-level job history, and light commercial context — notes, invoices, quotes — into one brief. An account manager asks how the whole tree is doing, not just one site, and Connie or the API answers from the rollup instead of stitching child summaries together by hand.

```mermaid
flowchart TD
    P(["Parent contact — e.g. Tesco PLC"])
    C1["Child contact: Tesco Watford"]
    C2["Child contact: Tesco Manchester"]
    C3["Child contact: Tesco Leeds"]
    S1[Customer Summary]
    S2[Customer Summary]
    S3[Customer Summary]
    J[Account-level jobs, notes, invoices, quotes]
    A(["Account Summary"])
    Q(["'How is this account doing across the whole tree?'"])
...
Account Summary sections use the same searchable, kept-current pattern as Customer Summary, scoped one level up: to the parent contact rather than a single site or child account.

**API paths**

| Action | Endpoint |
|--------|----------|
| Fetch full Account Summary for a parent contact | `GET /search/account-summary/by-parent/{company_id}/{contact_id}` |
| Search Account Summary sections by meaning | `POST /search/account-summary-sections` (optional section key and contact filters) |

<Tip>
Pass the ID of any child contact to `get_account_summary` (the Connie tool behind this) and it resolves up to the parent automatically, so you do not need to look up the parent contact first.
</Tip>
...
| The whole account rolled up across every child site | `GET /search/account-summary/by-parent/{company_id}/{contact_id}` |
