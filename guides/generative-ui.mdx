...
<Note>
**Preview.** The `@vh3/field-service-ui` package is in preview. The components, theming system, and contracts shown below are the targeted release surface. Get in touch if you would like to use it before the public release.
</Note>

## Why this exists

Most AI products either return prose or return raw JSON and leave the rendering problem to the consumer. Both are a problem in field service operations.

Prose alone is hard to act on. A coordinator reading a paragraph about an account's SLA performance cannot drill into the underlying jobs, cannot trust the numbers without verification, and cannot quickly compare the picture to other accounts. Raw JSON is unusable to anyone who is not building the rendering layer themselves, and produces inconsistent interfaces across every team that tries.
