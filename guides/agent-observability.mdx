...
sequenceDiagram
    ...
    participant C as Connie
    participant T as Tool (e.g. jobs_aggregate)

    U->>C: "Which engineers had the most repeat visits?"
    C->>T: Call tool with scope
    T-->>C: Structured result (toolCallOutputs)
    C-->>U: Natural-language answer + toolsUsed + toolCallOutputs + usage
    Note over U,C: Assistant text is the interpretation.<br/>toolCallOutputs is the evidence.
```
...
`jobs_aggregate`, `job_feed`, and related tools return **counts, groups, and rows** with labels suitable for display. Connie is instructed to prefer correct **time axes** for field work (actual start/end vs planned vs created), to count late jobs with punctuality filters (`finishedLate` / `startedLate`) rather than average deltas, and to note partial periods when comparing weeks.
