...
```mermaid
flowchart TD
    subgraph atQuestionTime["Most AI tools: the work starts when you ask"]
        direction TB
        A1(["You ask a question"]) --> A2[Search raw records]
        A2 --> A3[Grab the closest-matching fragments]
        A3 --> A4(["Answer from whatever surfaced"])
    end

    subgraph atIngestTime["VH3 AI: the work happens before you ask"]
        direction TB
        B1(["Job lands"]) --> B2[Enriched once: fault, outcome, relationships]
        B2 --> B3[Held in the prepared operational picture]
        B4(["You ask a question"]) --> B3
        B3 --> B5(["Answer grounded in what was already prepared"])
    end
```
