...
<Card title="Do this in Connect" icon="laptop" href="/connect/cases">
  Sentinel outcomes often land as cases and notifications in VH3 Connect. There is no separate Sentinels screen in the app today.
</Card>
...
```mermaid
flowchart LR
    S[Sentinel crosses threshold] --> D[Digest or API result]
    D --> C[Case opened]
    C --> T[Team notified]
    T --> B[Connie drafts briefing with citations]
    B --> H[Coordinator reviews and acts]
```
