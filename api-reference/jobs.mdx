---
title: Jobs
description: Job feed, account job feed, punctuality and follow-on work filters, and job details
---
...
Returns a paginated feed of enriched jobs for your company. Supports filtering by entity, status, punctuality, follow-on work, date range, and sort field. All filters are optional and composable as AND conditions. Each row includes `followUp` (`[]` or documented return-visit / parts-on-order lines).
...
| `has_follow_up` | boolean | No | false | `true` = only jobs with a non-empty `followUp`. Unset or `false` keeps the mixed feed |
| `include_vh3_ai` | boolean | No | false | `true` = append get-by-id-shaped `vh3_ai` on each job. Unset or `false` omits the key |
...
<Note>
`followUp` is a string array derived at read time from `technicalCore.follow_up`. Empty `[]` when there is no documented follow-on work. The Job object has no `hasFollowUp` field. `has_follow_up=true` paginates after the filter, so a page of 20 is 20 matching jobs and `totalJobs` / `totalPages` match that subset. `include_vh3_ai=true` appends get-by-id `vh3_ai` (duplication with top-level `followUp` is expected). Worksheets stay on get-by-id.
</Note>
...
**Example: jobs with documented follow-on work**

```bash
curl -G "https://api.vh3connect.io/api:kP8T1CK7/jobs/feed" \
  --data-urlencode "company_id=your-company-id" \
  --data-urlencode "api_key=your-api-key" \
  --data-urlencode "has_follow_up=true" \
  --data-urlencode "page_size=20"
```

**Example: list rows with get-by-id AI enrichment**

```bash
curl -G "https://api.vh3connect.io/api:kP8T1CK7/jobs/feed" \
  --data-urlencode "company_id=your-company-id" \
  --data-urlencode "api_key=your-api-key" \
  --data-urlencode "include_vh3_ai=true" \
  --data-urlencode "page_size=20"
```
...
<Note>
Each job includes `followUp` (string array) derived from `technicalCore.follow_up`. Empty `[]` when there is no documented follow-on work. `include_vh3_ai=true` appends get-by-id `vh3_ai` (duplication with top-level `followUp` is expected).
</Note>
...
- **Follow-on work across an account**, set `has_follow_up=true` to list only jobs with documented return visits or parts on order.
- **AI enrichment on the list**, set `include_vh3_ai=true` to include get-by-id-shaped `vh3_ai` without a second round-trip.
