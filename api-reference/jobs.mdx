...
| `compact` | boolean | No | false | Strip nulls/empties for LLM consumption |
...
<Note>
This endpoint resolves the `HAS_PARENT` graph relationship. If you pass a child contact ID, the parent is found and all sibling contacts are included. If you pass a parent, all children are included. Standalone contacts return only their own jobs.
</Note>
