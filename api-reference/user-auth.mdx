...
<Note>
  Need to manage users from a back-end script or integration (no user session)? Use the [Users](/api-reference/users) endpoints on the main VH3 AI API with `company_id` + `api_key`. Those endpoints are **server-side only:** the API key must never appear in client-side code.
</Note>
...
```bash
curl -G "https://api.vh3connect.io/api:lBQnyyZL/auth/me" \
  -H "Authorization: Bearer eyJhbGciOi..."
```
...
```bash
curl -G "https://api.vh3connect.io/api:lBQnyyZL/users/list" \
  -H "Authorization: Bearer eyJhbGciOi..." \
