...
Semantic search over job outcomes with Neo4j graph enrichment. Same request body and filters as `POST /search/outcomes`. Adds `related_works` grouped by job type, engineer, and site for each hit set.
...
- Slower than `/search/outcomes` due to Neo4j enrichment (~500ms–1s additional).
- `related_works` keys are derived from returned hits only, not the full matching corpus.
- Returns `502` if vector search or graph enrichment fails.
...
Semantic search over job intake text with knowledge graph enrichment, returns richer context by joining graph relationships to each result.
...
- Minimum query length is 2 characters. Accepts any entity Id such asd contact, job, job_group_id, reference, resource etc
