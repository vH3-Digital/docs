...
GroupBy: status, result, typeId, categoryId, resourceId, siteKey,
...
Prefer `filters.finishedLate=true` / `filters.startedLate=true` with
`metric=job_count`. Early aliases (`finishedEarly`, `startedEarly`) also
work. Finish/overrun → `timeAxis=actualEndAt`; start/arrival →
`actualStartAt`. Never estimate late counts from avg delta metrics.

For worst-N late lists: GET `/jobs/feed` with the same filters,
matching `dateField`/`dateFrom`/`dateTo`, `sortBy=endDeltaMins` (or
`startDeltaMins`), `direction=desc`.

## timeAxis: CRITICAL default

Default `timeAxis = "actualStartAt"` for any retrospective question.
Other values: actualEndAt (completions / finish punctuality),
plannedStartAt (forward-looking only, includes phantom jobs), createdAt
(intake only), scheduledAt (dispatching).
