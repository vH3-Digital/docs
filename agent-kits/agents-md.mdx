...
| "Late jobs / finished late / started late / overrun" | POST /aggregate/jobs | `metric=job_count` + punctuality filters (`finishedLate` / `startedLate`). Pair finish questions with `timeAxis=actualEndAt`. |
| "Worst / top N late jobs" | GET /jobs/feed | Same punctuality filters + matching `dateField`/`dateFrom`/`dateTo`, `sortBy=endDeltaMins` (or `startDeltaMins`), `direction=desc`. |
...
groupBy values: status | result | typeId | categoryId | resourceId | siteKey |
...
## Punctuality filters (aggregate + job feed)

Prefer positive late aliases in filters:
- `finishedLate=true` — finished after planned end
- `startedLate=true` — started after planned start
- `finishedEarly=true` / `startedEarly=true` — on or before plan

Combine with AND (e.g. started early AND finished late → overrun signal).
Use `metric=job_count` for counts. Never estimate late volume from
`avg_start_delta_mins` / `avg_end_delta_mins`.

timeAxis pairing:
- Finish / late / overrun questions → `actualEndAt`
- Start / arrival questions → `actualStartAt`

Job feed ranking: `sortBy=endDeltaMins` or `startDeltaMins`,
`direction=desc`. Null deltas sort last. Response fields remain
`finishedEarly` / `startedEarly` plus `startDeltaMins` / `endDeltaMins`
