...
- How General Email and Portal Email ingest results route downstream (section 6.8)
...
|---|---|---|---|---|
| API Key | `apiKey` | Yes | — | Sent as a header for BigChange operations and in the request body for VH3 AI Intelligence operations. |
| Company ID | `companyId` | Conditionally | — | Required for VH3 AI Intelligence operations and single-record BigChange operations. |
...
| Email (VH3 AI) | `email` | Fast–Medium | Email triage: classify, general ingest, portal ingest, batch classify, list rules |
...
| "Extract job data from this general inbox email" | `email` | `ingestGeneralEmail` |
...
|---|---|---|
| `listJobFeed` | `status` (multi), `dateField`, `dateFrom`, `dateTo`, `sortBy`, `direction`, `finishedLate` / `startedLate` / `finishedEarly` / `startedEarly`, `hasFollowUp`, `includeVh3Ai`, `contactId`, `resourceId`, `typeId` | Default `dateField`/`sortBy`: `createdAt`. Use `sortBy=endDeltaMins` for worst late finishes. `hasFollowUp: true` lists only jobs with a non-empty `followUp`. `includeVh3Ai: true` appends get-by-id `vh3_ai`. Add `simplify: true` to strip nulls. |
| `listAccountJobFeed` | `contactId` (required — any in hierarchy), same punctuality/date/sort/follow-up filters as `listJobFeed` | Traverses full parent-child account tree automatically |
...
| Operation | Key inputs | Notes |
|---|---|---|
| `generateReport` | `reportType`, `date`, `includeNarrative`, `sections` | Narrative adds 2–5 s. Omit `sections` to include all. `account_monthly` also needs `contactId` (optional `month`). |
| `listReportSections` | — | Discover section IDs for selective report generation |

**Report types**: `start_of_day`, `midday`, `close_of_business`, `day_review`, `start_of_week`, `midweek`, `end_of_week`, `account_monthly`.
...
| `ingestGeneralEmail` | Runs a non-portal email through full extraction and review. Uses the same fields and attachment options as Portal Ingest. |
| `ingestEmail` | Extracts structured job data from recognised FM portal emails. Returns resolved entities and a `status` field. |
...
Both ingest operations return the same status values. Always switch on `status` downstream:

| Status | Meaning |
|---|---|
| `auto_created` | New work passed the confidence checks. Use the returned job preview and creation result. |
| `classified` | The email was understood, but it does not require a new job. |
| `pending_review` | Customer, site, type, or other job details need human review. |
| `unprocessable` | The email did not contain enough reliable information to continue. |

<Note>
**Attachments:** General Email and Portal Email ingest accept files by URL or from n8n binary data. Supported work-order attachments can be sent with the email, so a separate **Extract From File** step is not required.
</Note>
...
Outlook trigger (new email)
  ...
  → VH3 AI: Email — Ingest Portal Email (emailText + preferredTypeIds from config)
  → Switch: ingestEmail.status
    ...
    → "pending_review": send to human approval queue (email / Slack)
    → "classified": record the result and exit
    → "unprocessable": log to spreadsheet / case and exit
```
...
Form trigger (file upload — xlsx or csv)
  ...
  → Split in Batches (configurable batch size, e.g. 5)
    → VH3 AI: Email — Ingest Portal Email (per row)
    → Switch: status → "auto_created" path (same as Pattern F) / record result
    → [loop continues]
...
- **`companyId` in credentials** is required for VH3 AI Intelligence operations and single-record BigChange operations.
